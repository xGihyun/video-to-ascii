...
pub fn main() !void {
    ...
    std.debug.print("Found {d} frames. Starting playback...\n", .{frame_files.items.len});
    for (frame_files.items) |frame| {
        try processFrame(allocator, frame);
...
fn bmpToAscii(data: []const u8, ascii_width: u32, ascii_height: u32) !void {
...
fn processFrame(allocator: std.mem.Allocator, frame_file: []const u8) !void {
    const file = try std.fs.cwd().openFile(frame_file, .{});
    defer file.close();
    ...
    try bmpToAscii(data, ASCII_WIDTH, ASCII_HEIGHT);
}
